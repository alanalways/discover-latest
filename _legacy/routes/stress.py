...
async def run_stress_test(req: StressTestRequest):
    ...
    if req.symbols:
        ...
        for sym in req.symbols[:20]:
            ...
            info: Dict[str, Any] = {}
            try:
                data = await stock_service.get_stock_data(sym, None, "1y") or {}
            ...
            current_price = (
                _safe_float(history[-1].get("close"), 100) if history else 100
            )
            ...
            results.append({
                ...
                "estimated_drawdown_pct": est_drawdown,
                "stressed_price": stressed_price,
            })

    avg_drawdown = (
        sum(r["estimated_drawdown_pct"] for r in results) / len(results)
        if results
        else scenario["tw_drawdown"]
    )
    ...
    return {
        "scenario": {
            ...
            "tw_drawdown": scenario["tw_drawdown"],
            "us_drawdown": scenario["us_drawdown"],
