"""
backend/services/smc_service.py
SMC/ICT Service — Smart Money Concepts 技術分析

從 _legacy/services/smc_service.py 移植。
提供 BOS/CHoCH/Order Block/FVG/Liquidity 計算。
"""
...
class SMCService:
    ...
    def _identify_structures(
        self, swings: List[Dict], history: List[Dict]
    ) -> List[Dict]:
        """識別 BOS 和 CHoCH"""
        structures = []
    ...
    def _identify_order_blocks(
        self, history: List[Dict], structures: List[Dict]
    ) -> List[Dict]:
        """識別 Order Blocks（供需區）"""
        order_blocks = []
    ...
    def _identify_liquidity(
        self, history: List[Dict], swings: List[Dict]
    ) -> List[Dict]:
        """識別流動性區域（等高/等低點群）"""
        liquidity = []
    ...
    def get_chart_markers(self, analysis: Dict) -> List[Dict]:
        """將分析結果轉換為圖表標記（Lightweight Charts 用）"""
        markers = []
