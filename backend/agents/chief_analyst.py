...
class ChiefAnalystAgent(BaseAgent):
    ...
    def _extract_target_prices(
        ...
        range_patterns = [
            # 帶單位：「目標價區間：40.0元 ~ 55.0元」或「目標價 40 ~ 55」
            r"目標價[區間範圍：:\s]*(?:約?\s*)?(?:NT?\$?\s*)?(\d[\d,.]*)\s*(?:元\s*)?[~～\-–—至到]\s*(?:NT?\$?\s*)?(\d[\d,.]*)",
            r"目標[價][：:\s]*(?:約?\s*)?(?:NT?\$?\s*)?(\d[\d,.]*)\s*(?:元\s*)?[~～\-–—至到]\s*(?:NT?\$?\s*)?(\d[\d,.]*)",
            # 「Target Price: 40 ~ 55」英文格式
            r"[Tt]arget\s+[Pp]rice[：:\s]*(?:NT?\$?\s*)?(\d[\d,.]*)\s*(?:元\s*)?[~～\-–—至to]\s*(?:NT?\$?\s*)?(\d[\d,.]*)",
        ]
