...
class SentimentAgent(BaseAgent):
    ...
    def analyze(
        ...
        Args:
            symbol:         股票代號
            market:         市場
            grounding_data: BatchGroundingAgent.fetch_all() 回傳的 sentiment_data 部分
            report_id:      關聯報告 UUID
        """
        sentiment_data = (grounding_data or {})
        result = self.run(
            report_id=report_id,
            symbol=symbol,
            market=market,
            sentiment_data=json.dumps(sentiment_data, ensure_ascii=False),
